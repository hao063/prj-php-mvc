...
function get_controller() {
    global $config;
    $controller = isset($_GET['contrl']) ? $_GET['contrl'] : $config['default_controller'];
    return $controller;
}
...
function get_action() {
    global $config;
    $action = isset($_GET['ac']) ? $_GET['ac'] : $config['default_action'];
    return $action;
}
...
function load($type, $name) {
    ...
    if ($type == 'app')
        $path = APP2PATH . DIRECTORY_SEPARATOR . "{$name}.php";
    if (file_exists($path)) {
        require "$path";
    } else {
