            ...
            <div class="card z-index-0 fadeIn3 fadeInBottom">
              <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                <div class="bg-gradient-primary shadow-primary border-radius-lg py-3 pe-1">
                  <h4 class="text-white font-weight-bolder text-center mt-2 mb-0">Đăng Nhập Người Quản Trị</h4>
                </div>
              </div>
              <div class="card-body">
                <form action="?mod=admin&contrl=admin&ac=login" method="post" role="form" class="text-start">
                  <div class="input-group input-group-outline my-3">
                    <label class="form-label">Tài khoản</label>
                    <input type="email" class="form-control" name="email">
                  </div>
                  <p class="validate-form"><?= !empty($data['email']) ? $data['email'] : ''?>&ensp;</p>
                  <div class="input-group input-group-outline mb-3">
                    <label class="form-label">Mật khẩu</label>
                    <input type="password" class="form-control" name="password">
                  </div>
                  <p class="validate-form"><?= !empty($data['password']) ? $data['password'] : ''?>&ensp;</p>
                  <div class="form-check form-switch d-flex align-items-center mb-3">
                    <input class="form-check-input" type="checkbox" id="rememberMe" name="rememberMe">
                    <label class="form-check-label mb-0 ms-2" for="rememberMe">Nhớ mật khẩu</label>
                  </div>
                  <div class="text-center">
                    <button type="submit" name="btn-login" class="btn bg-gradient-primary w-100 my-4 mb-2">Đăng nhập</button>
                  </div>
                </form>
              </div>
            </div>
